...
mat='_n42_m270_circ';
date='20180409_';
file_name=strcat(date,'axial_homopolar_bng_int_drive_',rev,mat);
%where project will be saved
server_path = strcat(cur_folder,'/tests');
%where commands are
...
try
    ...
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/1_param_revJ.py")')); 
    ...
    disp('mesh info OK')
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/4_geom_revI_circular.py")'));
    disp('geom OK')
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/5_mat_def.py")'));
    disp('material def ok')
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/6a_elec_circuit_saddle_wdg_drive_revB.py")'));
    disp('concentric coils for drive ok')        
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/6b_bearing_coil_toroidal_revE.py")'));
    disp('toroidal coils for bearing ok')        
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/7_assign_revF_circular.py")'));
    disp('assign ok')
...
%     disp('radial homopolar bearing ok')    
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/8_sensors_revE.py")'));
    disp('sensors ok')
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/9_make_scenarios_passive_revD.py")'));
    disp('make passive scenario ok')
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/10_make_scenarios_active_revD.py")'));
