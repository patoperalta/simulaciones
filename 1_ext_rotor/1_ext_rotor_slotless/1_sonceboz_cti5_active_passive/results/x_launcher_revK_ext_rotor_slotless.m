...
typ='ext_rotor_slotless_n42_no12';  %name
p=2;                                %pole pairs
...
%%copy characteristics of motors alpha, beta, d_agap, r_rot_out
xi_torque{2} = out(:,1:4);
...
p_joule_loss=out(:,10);   %save copper losses, will add on top the pcu_ed and p_fe
...
feasible = feasible .* (j_bng_startup < jmax);
feasible(feasible==0)=nan;
xi_stability{2}(:,10)=feasible;
xi_force{2}=out;
xi_force{1}=cols;
clear cols out j_bng_startup
...
mrkr=['x','o','d','*'];
%%load once again torque file, to obtain v_pm
load(name{7});
for i = 1 : 5       %length(beta)           %%changes marker size
    for j = 1 : 4   %length(dairgap)        %%different edge color 
        for k = 1 : 1   %length(rout)       %%before it was size, but it is now a constant
            for l = 1 : 4 %length(alpha)    %%changes face color
                %% define criteria to filter points
                %        alpha   beta    dagap     r out is not analyzed
                crit = [1.2+l*.2  i/10  j/2+1 ];
                %% filters
...
%% define numerical output
out=raw(:,1:4); %alpha,beta,dgap,rmot
out(:,5)=sqrt(raw(:,9).^2+raw(:,10).^2)./raw(:,5); %sqrt(fx_st^2+fy_st^2)/dx
...
%% define numerical output
out=raw(:,1:4); %alpha,beta,dgap,rmot
out(:,5)=sqrt(raw(:,10).^2+raw(:,11).^2)./raw(:,5); %sqrt(fx_st^2+fy_st^2)/dx
...
headers=headers';
%%read numerical data
raw=xlsread(filename); %(:,5) raw results, xlsread magically discards first row
...
vfe_st=out(:,5); %volume of stator in mm^3, will then be projected
fmec_max=(out(:,13)./out(:,12))*1/(2*pi); % Pmec / T = omega_max = 2 pi f_max
raw=[raw a_cu]; %(:,6)
headers=[headers 'A_CU'];
clear typ 

%%calculate height of stator and therefore lengthcols where losses occur
    %rrotout *  beta     /    alpha_h
h_st=2*raw(:,4).*raw(:,2)./raw(:,1);
raw=[raw h_st]; %(:,7)
headers=[headers 'H_ST'];

%% input parameters of coils for losses
...
% copper area fn
w_w=floor(a_cu*1e-6/(pi*w_d_cu.^2/4)*1/w_kcu); %a_cu is already per coil, number of turns of each coil
raw=[raw w_w];  %(:,8)
headers=[headers 'W_turns'];

%% calculate pcu_ed for each motor
%%define directory
dir = strcat('rev',rev,'\','1_b_fields_out_rev',rev);
    ...
    for k = 1 : 4 % d_agap
        for j = 1 : 5 % beta
            for i = 1 : 4 %alpha
                ...
                filename=strcat(...
                    'b_airgap_alpha',num2str(2*i+12),...
                    '_beta_0',num2str(j),...
                    '_dgap_',num2str(50*k+100),...
                    '_rout_',num2str(25*l+50));
                %%execute calculation of xi
                xi(n)=xi_b_airgap_hmax_revC(strcat(dir,'\',filename),w_p,harm_order_max,'B19:H1042');
                %%increase counter
                n=n+1;
            end
        end
...
lbar=xlsread(strcat('rev',rev,'\',mot,'_0_LBAR_rev',rev,'.csv'));%,'D2:D37');
lbar=lbar(:,4);
    ...
    for k = 1 : 4 % d_agap
        for j = 1 : 5 % beta
            for i = 1 : 4 %alpha
                %%define name
                filename=strcat(...
                    'h_alpha',num2str(2*i+12),...
                    '_beta_0',num2str(j),...
                    '_dgap_',num2str(50*k+100),...
                    '_rout_',num2str(25*l+50));    
                %%import
                b_h=xlsread(strcat(dir,'\',filename),1,'B17:F1040');
                %define what is important for us
                h=b_h(:,1);
                b=b_h(:,5); 
                %integrate
                int_bh_bed(n,1:2)=[trapz(h,b.^fe_props(2)) trapz(h,b.^fe_props(4))];
                %count on more
                n=n+1;
            end
        end
...
% p / f^i = c^i * rho_fe * v_fe/h_st * int_bi
ph_f=fe_props(1)*fe_props(5)*vfe_st.*1e-9/(h_st*1e-3).*int_bh_bed(:,1); %volume is in mm^3 and heigh in mm
ped_f2=fe_props(3)*fe_props(5)*vfe_st.*1e-9/(h_st*1e-3).*int_bh_bed(:,2); %volume is in mm^3 and heigh in mm
...
p_speed_n_max=pcu_fel2.*(w_p*fmec_max).^2+ph_f.*(w_p*fmec_max)+ped_f2.*(w_p*fmec_max).^2;
