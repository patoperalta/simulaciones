...
cur_folder=pwd;
rev='3';
mat='_constant_delta_ecce2018_n42_no12';
date='20180524_';
file_name=strcat(date,'ext_rotor_rev',rev,mat);
...
try
    ...
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/1_param_revI.py")')); %%%for rev 1 of mats
    ...
    disp('mesh info OK')
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/4_geom_revC_circ.py")'));
    disp('geom OK')
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/5_mat_def.py")'));
    disp('material def ok')
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/6_output_values.py")'));
    disp('out values ok')    
