...
lastInstance = ParameterGeom(name='N : rotor speed in rpm',
...
Scenario(name='PJ_ROT_'+str(r_rot))

startMacroTransaction()

Scenario['PJ_ROT_'+str(r_rot)].addPilot(pilot=MultiValues(parameter=VariationParameter['ANGPOS_ROTOR'],
                                                  intervals=[IntervalStepValue(minValue=0.0,
                                                                               maxValue=60.0,
                                                                               stepValue=1.0)]))

Scenario['PJ_ROT_'+str(r_rot)].addPilot(pilot=MultiValues(parameter=VariationParameter['ALPHA_H'],
                                                  intervals=[IntervalStepValue(minValue=alpha_min,
                                                                               maxValue=alpha_max,
                                                                               stepValue=d_alpha)]))

Scenario['PJ_ROT_'+str(r_rot)].addPilot(pilot=MultiValues(parameter=VariationParameter['BETA'],
                                                  intervals=[IntervalStepValue(minValue=beta_min,
                                                                               maxValue=beta_max,
                                                                               stepValue=d_beta)]))

Scenario['PJ_ROT_'+str(r_rot)].addPilot(pilot=MonoValue(parameter=VariationParameter['D_ST'],
                                                value=dst))

Scenario['PJ_ROT_'+str(r_rot)].addPilot(pilot=MonoValue(parameter=VariationParameter['L_SLOT'],
                                                value=lslot))

Scenario['PJ_ROT_'+str(r_rot)].addPilot(pilot=MonoValue(parameter=VariationParameter['N_PROP'],
                                                value=n))

Scenario['PJ_ROT_'+str(r_rot)].addPilot(pilot=MonoValue(parameter=VariationParameter['R_ROT_OUT'],
                                                value=r_rot))

Scenario['PJ_ROT_'+str(r_rot)].addPilot(pilot=MonoValue(parameter=VariationParameter['W_SLOT'],
                                                value=wslot))
