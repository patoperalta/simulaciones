...
mat='_n42_no12';
date='20180518_';
file_name=strcat(date,'int_rotor_slotted_airgap_rev',rev,mat);%,'_rout_14');
...
%     rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/1_param_revB.py")'));  %%%revA
    ...
    disp('mesh info OK')
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/4_geom_revE.py")'));
    disp('geom OK')
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/5_mat_def.py")'));
    disp('material def ok')
	rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/6_coils_concentric_revF.py")'));
    disp('coils ok')
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/7_assign_revB.py")'));
    disp('assign ok')
