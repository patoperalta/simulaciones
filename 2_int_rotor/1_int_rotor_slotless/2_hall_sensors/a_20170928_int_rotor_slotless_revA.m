...
mat='_n42_no12';
date='20180810_';
file_name=strcat(date,'int_rotor_slotless_rev',rev,mat);%,'_rout_14');
...
% rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/1_param_revA.py")'));
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/1_param_revF.py")'));  
    ...
    disp('mesh info OK')
%     rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/4_geom_revC.py")'));
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/4_geom_revC_circular.py")'));
    disp('geom OK')
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/5_mat_def.py")'));
    disp('material def ok')
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/6_elec_circuit_toroidal_revF.py")'));
    disp('toroidal coils ok')    
%     rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/7_assign.py")'));
...
%     disp('make active scenario ok')    
    rsi.fluxmp.FMP.FMP_executeJythonCommand (serverUid,strcat('executeBatchSpy("',python_path,'/11_make_scenarios_sensors.py")'));
    disp('make sensor scenarios ok')        
%     disp('meshing...')
%     tic
