...
f = open(dir+mot+'0_brms_a_around_teeth_rev'+rev+'.csv','w')
...
## open the file and
f = open(dir+mot+'_74_JF_rev'+rev+'.csv','w')
## writer the header
f.write(headers) #Give your csv text here.
## Python will convert \n to os.linesep
f.write('\n')
...
for m in range(0,len(cat)):
	for l in range(0,len(r_mot)):
		for k in range(1,len(d_gap)-2):		#range(0,len(d_gap)):
			for j in range(0,len(beta)-4):	#range(0,len(beta)):
